...
//בעת לחיצה על יצירת פתק בטופס שלנו, תופעל הפונקציה.
//הפונקציה מגדירה את ערכי הקלט כמשתנים

//מכאן, הפונקציה תפרמט את התאריך שהוזן לתאריך שמוצג באופן מובן (תהפוך את סדר המספרים) ותיצור את הפתק בעזרת פונקציה נוספת.
function prepareToCreateTask() {
    ...
    //מכיוון שאנו רוצים לוודא את מגבלות התוכן - שתוכן המשימה והתאריך אכן מוזנים, 
    //ושהמשימה מוגדרת לעתיד, נפעיל תהליך ולידציה בעזרת ערך בוליאני שמוגדר מלכתחילה ל-אמת.
    //אם התוכן יעבור את הולידציות בהצלחה, הערך שהגדרנו ישאר אמת ויווצר לנו פתק.
    ...
    //     actuallySetAReminder(reversedDateOfReminder, timeOfReminder, notesArr.length ,contentOfTask);
    // }
}

// function actuallySetAReminder(reversedDateOfReminder, timeOfReminder, noteNum , contentOfTask){

//     var reminderObject = {"Note number": noteNum ,"Reminder is set to": reversedDateOfReminder + " at-" + timeOfReminder};
//     reminderArr.push(reminderObject);
...
function addNewNoteToDOM (contentOfTask, timeOfTask, reversedDate) {
    ...
    divTag.setAttribute("class", "aba col-12 col-sm-6 col-md-2 col-lg-2 col-xl-2");
    divTag.innerHTML =
        `<div class="wrapper">   
...
//מוודא שאם הזנו את היום הנוכחי ולא הזנו שעה - הולידציה עדיין תעבור
    if (inputHour==""                                        &&
